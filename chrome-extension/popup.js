    ...
    async function fillMetadata() {
      function fillMetadataInjected() {
        (() => {
          ...
          function mapOptions(options) {
            return queryElements(options, 'option')
                .reduce((total, optionElement) => {
                  total.set(
                      optionElement.innerText.trim(), optionElement.value);
                  return total;
                }, new Map());
          }
          ...
          /**
           * Transform a summary by wrapping it in a <blockquote> and linking
           * the original work/authors.
          ...
          function transform(summary, title, url, authors) {
            const newSummary = '<blockquote>' + summary +
                '</blockquote>Podfic of ' + link(url, title) + ' by ';
            const newAuthors =
                Array.from(authors)
          ...
          chrome.storage.sync.get(
              ['metadata', 'options', 'workbody'], ({
                ...
                // Find the warning check boxes, and check all the ones that
                // apply.
                const warningBoxes = mapInputs(queryElements(
                    queryElement(newWorkPage, 'fieldset.warnings'), 'input'));
                warningBoxes.set(
                ...
                // Find the fandom text input, and insert a comma-separated list
                // of fandoms. Tell ao3 we did so.
                const fandomInput = queryElement(
                    queryElement(newWorkPage, 'dd.fandom'), 'input');
                setTagsInputValue(fandomInput, metadata['fandoms'].join(', '));

                // Find the category check boxes, and check all the ones that
                // apply.
                const categoryBoxes = mapInputs(queryElements(
                    queryElement(newWorkPage, 'dd.category'), 'input'));
                metadata['categories'].forEach(
                    category => categoryBoxes.get(category).checked = true);

                // Find the relationship text input, and insert a
                // comma-separated list of relationships. Tell ao3 we did so.
                const relationshipInput = queryElement(
                    queryElement(newWorkPage, 'dd.relationship'), 'input');
                setTagsInputValue(
                    relationshipInput, metadata['relationships'].join(', '));

                // Find the character input, and insert a comma-separated list
                // of characters. Tell ao3 we did so.
                const characterInput = queryElement(
                    queryElement(newWorkPage, 'dd.character'), 'input');
                setTagsInputValue(
                    characterInput, metadata['characters'].join(', '));

                // Find the freeform tags input, and insert a comma-separated
                // list of freeform tags. (potentially auto-adding "Podfic" and
                // "Podfic Length" tags) Tell ao3 we did so.
                const additionalTagsInput = queryElement(
                    queryElement(newWorkPage, 'dd.freeform'), 'input');
                ...
                setTagsInputValue(
                    additionalTagsInput, metadata['freeformTags'].join(', '));

                // Set the title.
                const titleInput = queryElement(
                    queryElement(newWorkPage, 'dd.title'), 'input');
                ...
                const summaryTextArea = queryElement(
                    queryElement(newWorkPage, 'dd.summary'), 'textarea');
                ...
                const parentCheckmark = queryElement(
                    queryElement(newWorkPage, 'dt.parent'), 'input');
                if (!parentCheckmark.checked) {
                  parentCheckmark.click();
                }
                const parentUrl =
                    queryElement(newWorkPage, '#work_parent_attributes_url');
                ...
                languageSelect.value =
                    languageOptions.get(metadata['language']);
