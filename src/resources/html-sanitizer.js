...
const HtmlSanitizer = new (function() {
  ...
  this.SanitizeHtml =
    ...
    if (input.indexOf('<body') == -1)
      input = '<body>' + input + '</body>';  // add "body" otherwise some tags
                                             // are skipped, like <style>
      ...
      if (node.nodeType == Node.TEXT_NODE) {
        newNode = node.cloneNode(true);
          ...
          (_tagWhitelist[node.tagName] || _contentTagWhiteList[node.tagName] ||
           (extraSelector &&
            node.matches(extraSelector)))) {  // is tag allowed?
              ...
              for (let s = 0; s < node.style.length; s++) {
                ...
                if (_cssWhitelist[styleName])
                  newNode.style.setProperty(
                      styleName, node.style.getPropertyValue(styleName));
              }
            } else {
              if (_uriAttributes[attr.name]) {  // if this is a "uri" attribute,
                                                // that can have "javascript:"
                                                // or something
        ...
        // remove useless empty spans (lots of those when pasting from MS
        // Outlook)
        if ((newNode.tagName == 'SPAN' || newNode.tagName == 'B' ||
             newNode.tagName == 'I' || newNode.tagName == 'U') &&
